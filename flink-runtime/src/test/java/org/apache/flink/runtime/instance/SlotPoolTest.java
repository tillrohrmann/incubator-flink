...
public class SlotPoolTest extends TestLogger {
	...
	public void testAllocateSimpleSlot() throws Exception {
		...
		try {
			...
			assertEquals(taskManagerLocation.getResourceID(), slot.getTaskManagerID());
	...
	public void testAllocationFulfilledByReturnedSlot() throws Exception {
		...
		try {
			...
			CompletableFuture<SimpleSlot> future1 = slotPoolGateway.allocateSlot(new SlotRequestID(), mock(ScheduledUnit.class), DEFAULT_TESTING_PROFILE, null, timeout);
			CompletableFuture<SimpleSlot> future2 = slotPoolGateway.allocateSlot(new SlotRequestID(), mock(ScheduledUnit.class), DEFAULT_TESTING_PROFILE, null, timeout);
			...
			assertEquals(slot1.getSlotContext().getAllocationId(), slot2.getSlotContext().getAllocationId());
		} finally {
			slotPool.shutDown();
		}
	}
	...
	public void testOfferSlot() throws Exception {
		...
		try {
			...
			CompletableFuture<SimpleSlot> future = slotPoolGateway.allocateSlot(new SlotRequestID(), mock(ScheduledUnit.class), DEFAULT_TESTING_PROFILE, null, timeout);
			assertFalse(future.isDone());
