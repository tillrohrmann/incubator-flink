...
public class RestClient {
	...
	private static void addPathsToEncoder(Collection<Path> paths, String attributeName, String contentType, HttpPostRequestEncoder encoder) throws HttpPostRequestEncoder.ErrorDataEncoderException, IOException {
		...
		String[] types = new String[paths.size()];
		boolean[] isText = new boolean[paths.size()];
		int index = 0;
		for (Path jar : paths) {
			...
			files[index] = new File(jar.makeQualified(jar.getFileSystem()).toUri());
			types[index] = contentType;
			isText[index] = false;
			index++;
		}
		LOG.trace("Adding files {} to request.", Arrays.toString(files));
		encoder.addBodyFileUploads(attributeName, files, types, isText);
	}
